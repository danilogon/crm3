...
import { X, Save, Calendar, MessageSquare, Paperclip, Upload, Download, Trash2, Plus } from 'lucide-react';
import { SeguroNovo, StatusSeguroNovo, Seguradora, Anexo } from '../../types';
import { formatarMoeda, calcularComissaoNova } from '../../utils/calculations';
...
interface SeguroNovoModalProps {
  seguroNovo?: SeguroNovo;
  onClose: () => void;
  onSave: (seguroNovo: SeguroNovo) => void;
}

const SeguroNovoModal: React.FC<SeguroNovoModalProps> = ({ seguroNovo, onClose, onSave }) => {
  const [formData, setFormData] = useState<SeguroNovo>(
    seguroNovo || {
      id: '',
      clienteId: '', // Será preenchido quando cliente for selecionado
      responsavel: '',
      inicioVigencia: new Date(),
      ramo: mockRamos[0],
      seguradoraNova: mockSeguradoras[0],
      premioLiquidoNovo: 0,
      percentualComissaoNova: 0,
      comissaoNova: 0,
      comissaoAReceber: 0,
      status: 'a_trabalhar',
      observacoes: [],
  ...
  useEffect(() => {
    ...
    if (formData.premioLiquidoNovo && formData.percentualComissaoNova) {
      ...
      // Verificar se novaComissao é válida antes de calcular comissaoAReceber
      const comissaoAReceber = !isNaN(novaComissao) && novaComissao > 0 
        ? calcularComissaoAReceber(novaComissao, formData.ramo)
        : 0;
      
      setFormData(prev => ({
        ...prev,
        comissaoNova: novaComissao,
        comissaoAReceber: comissaoAReceber
      }));
    }
  }, [formData.premioLiquidoNovo, formData.percentualComissaoNova, formData.ramo]);

  const handleInputChange = (field: string, value: any) => {
    if (field === 'status' && value === 'perdido' && formData.status !== 'perdido') {
      // Solicitar motivo quando status mudar para "perdido"
      // Definir primeiro motivo ativo como padrão
      const primeiroMotivo = configuracoesMotivos.motivosSeguroNovo
        .filter(m => m.ativo)
        .sort((a, b) => a.ordem - b.ordem)[0];
      setMotivoPerda(primeiroMotivo?.id || '');
  ...
  const handleSave = () => {
    ...
    if (!formData.responsavel || !formData.clienteId || !formData.premioLiquidoNovo) {
      alert('Por favor, preencha todos os campos obrigatórios e vincule um cliente.');
      return;
    }

    const seguroNovoAtualizado = {
      ...formData,
      id: formData.id || Date.now().toString(),
      atualizadoEm: new Date(),
      criadoEm: formData.criadoEm || new Date()
    };
    
    onSave(seguroNovoAtualizado);
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg max-w-5xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-800">
            {seguroNovo ? `Seguro Novo - ${formData.nomeCliente}` : 'Novo Seguro'}
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
          {/* Campos Editáveis */}
          <div>
            <h3 className="font-medium text-gray-800 mb-4">Informações do Seguro</h3>
              ...
              {clienteSelecionado && (
                <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-md">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="font-medium text-green-800">{clienteSelecionado.nome}</p>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  type="date"
                  value={formData.inicioVigencia.toISOString().split('T')[0]}
                  onChange={(e) => handleInputChange('inicioVigencia', new Date(e.target.value))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                />
              </div>
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    const ramo = ramos.find(r => r.id === e.target.value);
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (seguradora) handleInputChange('seguradoraNova', seguradora);
                  }}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                >
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Comissão a Receber (Calculado)
                </label>
                <div className="space-y-1">
                  <input
                    type="text"
                    value={formData.comissaoAReceber ? formatarMoeda(formData.comissaoAReceber) : ''}
                    disabled
                    className="w-full px-3 py-2 border border-gray-300 rounded-md bg-green-100 text-green-700 font-medium"
                  />
                  {formData.ramo && (
                    <div className="text-xs text-green-600 font-medium">
                      {formData.ramo.tipoComissaoSeguroNovo === 'valor_fixo' 
                        ? `Valor fixo: ${formatarMoeda(formData.ramo.valorFixoSeguroNovo)} por seguro fechado`
                        : `${formData.ramo.percentualComissaoSeguroNovo}% da comissão gerada`
                      }
                    </div>
                  )}
                </div>
      ...
      {/* Modal de Motivo para Perda */}
      {showMotivoModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-60">
          <div className="bg-white rounded-lg max-w-md w-full shadow-2xl">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-800">Motivo da Perda</h3>
              <button
            ...
            <div className="p-6 space-y-4">
              <p className="text-sm text-gray-600">
                Por favor, informe o motivo pelo qual este seguro novo foi perdido:
              </p>
              ...
              {configuracoesMotivos.motivosSeguroNovo.find(m => m.id === motivoPerda)?.nome.toLowerCase().includes('outros') && (
                <div className="mt-4">
...
export default SeguroNovoModal;